...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_FONT = Color.parseColor("#673AB7");
    private static int COLOR_TITLE_HEADER = Color.parseColor("#673AB7");
    ...
    public SuspensionDecoration(Context context, List<? extends ISuspensionInterface> datas) {
        ...
        mPaint = new Paint();
        mBounds = new Rect();
        mTitleHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 72, context.getResources().getDisplayMetrics());
        mTitleFontSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 22, context.getResources().getDisplayMetrics());
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {//最先调用，绘制在最下层
        ...
        //nt baseline = (getMeasuredHeight() - fontMetrics.bottom + fontMetrics.top) / 2 - fontMetrics.top;

        mPaint.getTextBounds(mDatas.get(position).getSuspensionTag(), 0, mDatas.get(position).getSuspensionTag().length(), mBounds);
        c.drawText(mDatas.get(position).getSuspensionTag(), padWidth + rectWidth * 3 + child.getPaddingLeft(), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);

        mPaint.setColor(COLOR_TITLE_HEADER);
        c.drawRect(left + padWidth, child.getTop() - params.topMargin - mTitleHeight + mBounds.height() / 2 + rectWidth / 2
                , left + rectWidth + padWidth, child.getTop() - params.topMargin - mBounds.height() / 2 - rectWidth / 2, mPaint);
    }
    ...
    /* @Override
     public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
         ...
         mPaint.setColor(COLOR_TITLE_FONT);
         mPaint.getTextBounds("行：" + tag, 0, tag.length(), mBounds);

         int rectWidth = UIUtils.dip2px(10);
         int padWidth = UIUtils.dip2px(128);
         c.drawText("行：" + tag, padWidth+child.getPaddingLeft() + rectWidth * 3,
                 parent.getPaddingTop() + mTitleHeight - (mTitleHeight / 2 - mBounds.height() / 2),
                 mPaint);

         // 绘制蓝色的头部框
         mPaint.setColor(COLOR_TITLE_HEADER);
         c.drawRect(padWidth, mBounds.height() / 2 + rectWidth / 2
                 , rectWidth+padWidth                              , parent.getPaddingTop() + mTitleHeight - mBounds.height() / 2 - rectWidth / 2, mPaint);
         if (flag)
